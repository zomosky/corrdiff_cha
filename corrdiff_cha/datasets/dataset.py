...
## change cpu use of dataloader
import psutil
import os

def set_worker_affinity(worker_id):
    p = psutil.Process(os.getpid())
    cpu_count = int(os.environ.get('OMP_NUM_THREADS', '2'))
...
def init_dataset_from_config(
    ...
    dataset_iterator = iter(
        torch.utils.data.DataLoader(
            ...
            worker_init_fn=None,
            # worker_init_fn=set_worker_affinity,
            **dataloader_cfg,
        )
    )
