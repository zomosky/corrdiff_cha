...
def datacrop(data,size):
    if len(data.shape) == 2:
        ...
        raise ValueError('data_input shape error, must >= 2')
###
...
def main(cfg: DictConfig) -> None:
    ...
    while not done:
        ...
        # Update weights.
        lr_rampup = cfg.training.hp.training_duration // 20  # ramp up the learning rate
        for g in optimizer.param_groups:
            if lr_rampup > 0:
                g["lr"] = cfg.training.hp.lr * min(cur_nimg / lr_rampup, 1)
            if cur_nimg >= lr_rampup:
                g["lr"] *= cfg.training.hp.lr_decay ** ((cur_nimg - lr_rampup) // (lr_rampup // 2))
            current_lr = g["lr"]
            if dist.rank == 0:
